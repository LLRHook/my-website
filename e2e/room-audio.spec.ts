...
/** Playwright's Chromium allows autoplay. Emulate the default policy: silence until the document's first real gesture. */
async function blockAutoplayUntilActivation(page: Page) {
  await page.addInitScript(() => {
    const Wrapped = window.AudioContext;
    // Automation evaluations can activate a document before hydration. Only real
    // pointer/key events should unlock this policy probe, just as the visitor would.
    let activated = false;
    const activate = (event: Event) => { if (event.isTrusted) activated = true; };
    for (const name of ["pointerdown", "pointerup", "keydown"]) {
      document.addEventListener(name, activate, { capture: true, passive: true });
    }
